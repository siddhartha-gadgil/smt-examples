# This is due to Anand Tadipatri
...
# this is the equations that the distances (rather, the squares of the distances) must satisfy
dist_eq = d(A, D) * d(B, E) * d(C, F) == d(D, B) * d(E, C) * d(F, A)
# %%s
#these are the "forward" and "backward" statements of the theorem
menelaus_thm_fwd = Implies(And(y_a > 0, are_collinear(D, E, F)), dist_eq)
menelaus_thm_bwd = Implies(And(y_a > 0, Or(one_not_in_bounds, three_not_in_bounds), dist_eq), are_collinear(D, E, F))
# %%
